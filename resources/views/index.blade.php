        ...
        <div class="container">
            <div class="row fullscreen d-flex align-items-center justify-content-center">
                <div class="banner-content col-lg-12">
                    ...
                    <p class="text-white"><h5 class="text-white"><i class="fa fa-search" aria-hidden="true"></i> &nbsp;
                        Search by categories</h5>
                    @foreach($searchByCategory as $id=>$searchByCategory)
                        <a href="{{ route('categories.show', $id) }}"
                           class="text-white">{{ $searchByCategory }}</a>@if (!$loop->last),@endif
                        @endforeach
                        </p>
                </div>
            </div>
...
@section('content')
    <div class="col-lg-8 post-list">
        ...
        <a class="text-uppercase btn btn-primary" href="{{ route('jobs.index') }}"> <i class="fa fa-spinner"
                                                                                       aria-hidden="true"></i> &nbsp;
            Load More Job Posts</a>
    </div>
